import { Q as QueryClientProviderWithDefault, c as createSOLQueryKeyWithNetwork, n as neverPersist, a as createSOLProgramQueryKey, e as ensureTWPrefix } from '../../dist/query-key-df02df0e.esm.js';
...
function programQuery(queryClient, sdk, address, type) {
  ...
  return {
    ...
    queryFn: async () => {
      invariant(sdk, "sdk is required");
      invariant(address, "Address is required"); // if the type is not passed in explicitly then we'll try to resolve it
...
function nftGetAllQuery(program) {
  return {
    queryKey: createSOLProgramQueryKey(program, ["getAll"]),
    queryFn: async () => {
...
function nftGetOneQuery(program, tokenAddress) {
  return {
    queryKey: createSOLProgramQueryKey(program, ["get", {
      tokenAddress
    }]),
    queryFn: async () => {
      invariant(program, "program is required");
...
function useMintToken(program) {
  ...
  return useMutation(async params => {
    invariant(program, "program is required"); // TODO switch this to use mintTo once that is exposed
...
function useLogin(config) {
  ...
  async function login(cfg) {
    ...
    queryClient.invalidateQueries(ensureTWPrefix(["user"])); // Redirect to the login URL with the encoded payload
...
export { ThirdwebProvider, ThirdwebSDKProvider, ThirdwebWrapperProvider, balanceQuery, claimConditionsQuery, dropTotalClaimedSupplyQuery, dropUnclaimedSupplyQuery, nftGetAllQuery, nftGetOneQuery, programAccountTypeQuery, programMetadataQuery, programQuery, tokenBalanceQuery, tokenSupplyQuery, useAuth, useBalance, useBurnNFT, useClaimConditions, useClaimNFT, useDropTotalClaimedSupply, useDropUnclaimedSupply, useLazyMint, useLogin, useLogout, useMintNFT, useMintToken, useNFT, useNFTs, useProgram, useProgramAccountType, useProgramMetadata, useSDK, useSetClaimConditions, useTokenBalance, useTokenSupply, useTransferNFT, useTransferToken, useUser };
